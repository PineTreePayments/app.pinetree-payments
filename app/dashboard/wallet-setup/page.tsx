...
function walletCreationStepMessage(step: WalletCreationStep) {
  ...
  if (step === "provisioning_wallet") return "Creating your PineTree Wallet..."
...
function extractDynamicUserEmail(dynamicUser: unknown) {
...
function PineTreeWalletRuntime() {
      ...
      if (!walletRes.ok) {
        ...
        if (typeof window !== "undefined") {
          ...
          if (setupKey) {
            ...
            if (setupStarted && json.profile?.status !== "ready") {
              setPendingSync(true)
  ...
  const dynamicUserEmail = useMemo(() => extractDynamicUserEmail(user), [user])
  ...
  const syncProfileFromDynamic = useCallback(async (options?: { autoEnableLightning?: boolean; requireBaseAndSolanaSigners?: boolean }) => {
    ...
    try {
      ...
      if (merchantEmail && dynamicUserEmail && dynamicUserEmail !== merchantEmail) {
        keepPendingSync = false
        const message = "Use the same email as your PineTree account to create your PineTree Wallet."
      ...
      if (!merchantEmail || !dynamicUserEmail || dynamicUserEmail !== merchantEmail) {
        keepPendingSync = false
        const skippedReason = !merchantEmail
          ? "missing_pinetree_merchant_email"
          : !dynamicUserEmail
            ? "missing_dynamic_user_email"
  ...
  }, [user, wallets, primaryWallet, dynamicWalletSearchList, dynamicNetworkAddresses, dynamicWalletRuntimeCount, waasRuntimeWallets.length, waasCredentialWalletSources.length, waasCredentialSignerWallets.length, repairInProgress, syncPineTreeManagedLightning, logWalletCreationStep, fetchProviderRailState, merchantEmail, dynamicUserEmail])
  ...
  useEffect(() => {
    ...
    setShowAuthFlow(true)
  }, [repairPendingAfterLogout, user, setShowAuthFlow, setShowDynamicUserProfile, logWalletCreationStep])
  ...
  const walletStatus = repairInProgress ? "Repairing" : walletProvisioningInProgress ? "Provisioning" : dynamicProfileReady ? "Ready" : repairOrSetupIncomplete ? "Setup incomplete" : "Not connected"
  ...
  const walletCreationMessage = identityMismatchError ? "" : walletCreationStepMessage(walletCreationStep)
  ...
  const walletCreationInProgress =
    ...
    walletCreationStep !== "failed" &&
    walletCreationStep !== "timeout"
  const showProvisioningRetryOnly = (walletCreationStep === "timeout" && Boolean(user)) || Boolean(identityMismatchError)
  ...
  function handleRetryWalletSetup() {
    if (identityMismatchError || (walletIdentityError && user)) {
    ...
    <>
      <article className="max-w-2xl rounded-[1.35rem] border border-blue-200/70 bg-[radial-gradient(circle_at_top_right,rgba(37,99,235,0.13),transparent_38%),linear-gradient(135deg,rgba(255,255,255,0.98),rgba(247,251,255,0.96))] p-5 shadow-[0_20px_55px_rgba(37,99,235,0.12)] backdrop-blur sm:p-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <WalletStatusPill
            label={dynamicProfileReady ? "Ready" : syncing ? "Saving..." : walletStatus}
            tone="blue"
            className="shrink-0"
          />
        </div>
          ...
          <div className="mt-4 flex flex-wrap items-start gap-2 rounded-xl border border-amber-200 bg-amber-50/80 px-3 py-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs leading-5 text-amber-800">
                PineTree account email: {identityMismatchError.merchantEmail}
              </p>
              <p className="mt-1 text-xs leading-5 text-amber-800">
                Sign out of the current Dynamic session, then enter your PineTree account email if Dynamic asks again.
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="mt-6 flex justify-start">
          {identityMismatchError ? (
            ...
            >
              {logoutPending ? "Preparing..." : "Use PineTree account email"}
            </button>
          ) : hasWallet || repairFailedIncomplete ? (
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={repairOrSetupIncomplete ? handleFinishWalletSetup : handleOpenWallet}
                disabled={syncing || walletCreationInProgress}
                className="h-10 rounded-lg bg-[#0052FF] px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700 disabled:opacity-60"
              >
                {repairOrSetupIncomplete ? "Finish PineTree Wallet setup" : "Open PineTree Wallet"}
              </button>
              {repairOrSetupIncomplete ? (
                <button
                  type="button"
