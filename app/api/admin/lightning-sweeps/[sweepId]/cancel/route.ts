...
import { cancelAdminLightningSweep } from "@/engine/adminLightningSweeps"

type RouteContext = { params: Promise<{ sweepId: string }> }

function message(error: unknown) {
  return error instanceof Error ? error.message : "Lightning sweep cancel failed"
}
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const sweep = await cancelAdminLightningSweep({ sweepId, adminId })
