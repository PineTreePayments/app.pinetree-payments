...
import { prepareOffRampWalletApprovalPreviewForMerchant } from "@/engine/offRampOperations"
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await prepareOffRampWalletApprovalPreviewForMerchant({
      merchantId,
      sessionId: id
    })
    ...
    return NextResponse.json(
      { error: getErrorMessage(error, "Failed to preview wallet approval") },
