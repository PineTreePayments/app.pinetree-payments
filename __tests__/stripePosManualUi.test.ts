...
const experienceSource = readFileSync("components/pos/PosCardPaymentExperience.tsx", "utf8")
...
describe("PineTree POS manual-entry UI", () => {
  it("initializes direct-charge Stripe.js with the platform key and authenticated account context", () => {
    ...
    expect(posSource).toContain("manualStripeAccountId={manualStripeAccountId}")
    expect(experienceSource).toContain("stripeAccountId={props.manualStripeAccountId}")
  })
  ...
  it("waits for PineTree state after Stripe.js confirmation", () => {
    ...
    expect(posSource).toContain('setCardView("processing")')
