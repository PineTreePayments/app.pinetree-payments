...
describe("dashboard checkout and developer navigation", () => {
  ...
  it("uses compact Developer cards as the only section selector", () => {
    ...
    expect(developer).toContain('label="Requires setup"')
    expect(developer).toContain("Shopify support is wired, but a Shopify app and storefront setup are required before install testing.")
    ...
    expect(developer).not.toContain("OAuth")
    expect(developer).not.toContain("shopify_connections")
