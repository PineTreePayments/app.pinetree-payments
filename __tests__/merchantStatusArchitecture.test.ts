...
describe("Merchant Status Architecture contract", () => {
  ...
  it("defines every current merchant status and the future dispute presentation", () => {
    ...
    expect(getPaymentDisplayStatus("EXPIRED")).toMatchObject({ label: "Expired", tone: "expired", icon: "x-circle" })
    expect(getPaymentDisplayStatus("INCOMPLETE")).toMatchObject({ label: "Canceled", tone: "canceled", icon: "x-circle" })
  ...
  it("reports refunds as adjustments instead of confirmed payments", async () => {
    ...
    expect(report.statusCounts).toEqual({ Refunded: 1, Waiting: 1, Canceled: 1 })
    expect(report.transactionsTable.map((row) => row.status)).toEqual(["Refunded", "Waiting", "Canceled"])
