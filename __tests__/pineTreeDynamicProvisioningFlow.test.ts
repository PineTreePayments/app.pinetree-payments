...
describe("PineTree Dynamic provisioning flow", () => {
  ...
  it("Open PineTree Wallet starts browser-to-server profile sync and logs the exact result", () => {
    ...
    expect(page).toContain("dynamicUserIdPresent: Boolean(user.userId)")
    expect(page).toContain("baseAddressPresent: Boolean(baseAddress)")
    expect(page).toContain("solanaAddressPresent: Boolean(solanaAddress)")
  ...
  it("mismatched Dynamic email is rejected before saving a PineTree Wallet profile", () => {
    ...
    expect(identityGuard).toContain("Use your PineTree account email to verify wallet access.")
  ...
  it("Create PineTree Wallet is hidden while a mismatched Dynamic session is active", () => {
    ...
    const ctaBlock = page.slice(
      page.indexOf('<div className="mt-auto flex justify-center pt-8">'),
