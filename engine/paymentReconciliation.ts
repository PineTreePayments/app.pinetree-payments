...
export async function repairTerminalPaymentForReconciliation(
  ...
  repairableStatuses: Array<"INCOMPLETE" | "FAILED"> = ["INCOMPLETE", "FAILED"]
): Promise<ReconciliationRepairResult> {
  ...
  if (currentStatus !== "INCOMPLETE" && currentStatus !== "FAILED") {
  ...
  try {
    ...
    await updatePaymentStatusInDb(paymentId, "PROCESSING", currentStatus)
  } catch (error) {
