...
export default function BaseWalletPayment({
  ...
  const dispatchPendingWalletAction = useCallback(async (source = "manual"): Promise<boolean> => {
    ...
    try {
      ...
      setBaseUsdcV4Status(
        kind === "usdc_approve"
          ? "Wallet was interrupted. Tap Continue in wallet to complete USDC authorization."
          : "Wallet was interrupted. Tap Continue in wallet to confirm payment."
      )
      setLocalError("")
      return false
    }
  ...
  const executeBaseUsdcV5Payment = useCallback(async (input: {
    ...
    try {
      ...
      if (capabilities.skipDelegatedBatch) {
      ...
      if (!capabilities.skipDelegatedBatch) try {
