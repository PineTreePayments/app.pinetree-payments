...
export default function SolanaWalletPayment({
  ...
  const handlePay = useCallback(async () => {
    ...
    try {
      ...
      // Optional trigger endpoint remains handled by parent onSuccess callback.
  ...
  const handlePayWithSolanaClick = useCallback(() => {
  ...
  return (
    <div className="space-y-3">
      ...
      {isPaying ? (
        ...
        <Button fullWidth disabled={connecting} onClick={handlePayWithSolanaClick}>
          {connecting
            ? "Connecting wallet..."
            : isIntentMode
              ? "Pay with Solana"
              : `Pay with Solana (${nativeAmount} SOL)`}
        </Button>
      )}
