...
- **Route files:** 259 `app/api/**/route.ts`
- **Exported method handlers:** 316
...
| `/api/admin/business-verification/simulate-approval` | POST | ADMIN | `requireAdminFromRequest` → `merchants.role == "admin"` | Platform-wide by role; **plus** a server-side `BRIDGE_ENVIRONMENT === "sandbox"` gate in the Engine and again in the provider client, both failing closed. Refused in production regardless of caller | Yes | [`app/api/admin/business-verification/simulate-approval/route.ts:20`](../../app/api/admin/business-verification/simulate-approval/route.ts#L20) | VERIFIED |
...
| `/api/onboarding/business-verification/agreement` | POST | DASHBOARD_SESSION | `requireOnboardingMerchantSession` (session only; API key → 403) | Merchant id from the session; the agreement reference is shape-bounded before storage and is never approval evidence | Yes | [`app/api/onboarding/business-verification/agreement/route.ts:26`](../../app/api/onboarding/business-verification/agreement/route.ts#L26) | VERIFIED |
...
| `/api/wallets/pinetree-wallet/bank-destinations` | GET | DASHBOARD_SESSION | `requireMerchantIdFromRequest` (Supabase JWT or `pt_live_` key) | Merchant id from the verified token; never from the body. Response is the Engine's masked projection — no provider identifier, no account number | No | [`app/api/wallets/pinetree-wallet/bank-destinations/route.ts:22`](../../app/api/wallets/pinetree-wallet/bank-destinations/route.ts#L22) | VERIFIED |
| `/api/wallets/pinetree-wallet/bank-destinations` | POST | DASHBOARD_SESSION | `requireMerchantIdFromRequest` (Supabase JWT or `pt_live_` key) | Merchant id from the verified token; never from the body. The routing/account numbers exist only for this request and are forwarded to the provider, never persisted or logged | Yes | [`app/api/wallets/pinetree-wallet/bank-destinations/route.ts:39`](../../app/api/wallets/pinetree-wallet/bank-destinations/route.ts#L39) | VERIFIED |
| `/api/wallets/pinetree-wallet/bank-destinations/[id]` | DELETE | DASHBOARD_SESSION | `requireMerchantIdFromRequest` (Supabase JWT or `pt_live_` key) | Merchant id from the verified token; the Engine read is filtered by merchant, so a foreign destination id resolves to 404 | Yes | [`app/api/wallets/pinetree-wallet/bank-destinations/[id]/route.ts:13`](../../app/api/wallets/pinetree-wallet/bank-destinations/[id]/route.ts#L13) | VERIFIED |
| `/api/wallets/pinetree-wallet/bank-withdrawals` | POST | DASHBOARD_SESSION | `requireMerchantIdFromRequest` (Supabase JWT or `pt_live_` key) | Merchant id from the verified token; the bank destination and settlement route are both resolved merchant-scoped server-side | Yes | [`app/api/wallets/pinetree-wallet/bank-withdrawals/route.ts:17`](../../app/api/wallets/pinetree-wallet/bank-withdrawals/route.ts#L17) | VERIFIED |
