...
### Payment
Every checkout session that receives a payment attempt can produce a `payment` object. The payment object tracks status, network, rail, amount, currency, and metadata. Public payment statuses are `open`, `processing`, `paid`, `failed`, `incomplete`, `expired`, and `canceled`; `paid` maps to PineTree's visible **Confirmed** state.
